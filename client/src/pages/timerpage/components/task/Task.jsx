...
export default function(props){
    function removeTask() {
        props.editTask(current => {
            if(props.current) {
                const removedCurrent = [...current]
                removedCurrent.shift()
                if(removedCurrent.length <= 0) return []
    ...
    return (
        <div className={`pomodoro-task flex ${props.current ? 'current-task' : ''}`}>
            ...
            {props.isEditing ? <button onClick={removeTask}>
            ...
            </button> : <p>{props.finished}/{props.max}</p>}
        </div>
    )
}